...
- Fiat on-ramp via a third-party provider.
...
### 6.11 Buy (fiat on-ramp)

MoonPay integration. The signed widget URL is generated by the backend, so the signing secret never reaches the client.

### 6.12 Settings, localisation and theme
...
| Announcements | `/news` |
| Fiat on-ramp | `/moonpay/getSignUrl` |
