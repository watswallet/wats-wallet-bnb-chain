<template>
    <!-- GERCEK origin (current_request.hostname, arka planin sender'dan cozdugu
         deger) EN BASKIN eleman -- TonConnectApprove.vue/TonSendTx.vue'deki
         AYNI kural (K5) VE AYNI alan: manifest BASKA bir origin'de
         barinabilir, orayi "origin" gibi gostermek tam da onlenmek istenen
         kimlik taklidi olurdu. UC onay ekrani da (bu, TonConnectApprove.vue,
         TonSendTx.vue) AYNI gorsel guven desenini (artik ApprovalShell.vue)
         paylasir -- kullanici NEREYE bakacagini bir kere ogrenir; sema
         (scheme) gorunurlugu GEREKLIYSE bu UCUNE BIRDEN, ayri bir kararla
         eklenmeli, tek ekranda SESSIZCE degil (fix round 1, kontrolor notu). -->
    <ApprovalShell
        chain="ton"
        :title="$t('dapps.tonConnect.signDataTitle')"
        :origin="hostname"
        :claimed-name="manifestName"
        :claimed-icon="manifestIcon"
    >
        <div class="bg-white dark:bg-[#131315] border border-slate-200 dark:border-white/5 rounded-xl p-3.5 flex flex-col gap-2 shadow-sm dark:shadow-none transition-colors duration-300">
...
const onayla = async () => {
    ...
    try {
        ...
        if (res?.success) {
            ...
            // res.error bir KOD'dur; ceviri BURADA yapilir (TonSendTx.vue ile AYNI
            // sozlesme, AYNI tablo). Ikinci bir kod->metin haritasi TUTULMAZ.
            // Taninmayan bir kod GIZLENMEZ: imza yolu onu ham haliyle gosterir --
            // sebepsiz birakmaktansa teknik bir dize daha iyidir.
            hata.value = res?.error ? tonSendErrorText(res.error, t) : t('dapps.tonConnect.signErrorGeneric')
        }
    } finally {
        loading.value = false
    }
