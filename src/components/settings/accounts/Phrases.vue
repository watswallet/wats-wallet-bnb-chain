<template>
    <div class="w-full h-full max-w-[420px] mx-auto flex flex-col bg-slate-50 dark:bg-[#09090b] text-slate-900 dark:text-white font-sans relative overflow-hidden selection:bg-rose-500/30 transition-colors duration-300">
        ...
        <div class="absolute top-0 left-0 right-0 h-32 bg-linear-to-b from-rose-500/5 dark:from-zinc-800/20 to-transparent pointer-events-none transition-colors duration-300"></div>
        ...
        <div class="flex items-center justify-between px-5 pt-5 pb-3 relative border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-transparent backdrop-blur-md z-10 transition-colors duration-300">
            ...
            <h1 class="text-lg font-bold tracking-tight text-slate-900 dark:text-white transition-colors duration-300">{{ $t('settings.account.phrases.title') }}</h1>
            ...
            <button 
                ...
                :title="$t('settings.account.phrases.tooltip_copy')" 
            >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-5 pt-5 flex flex-col gap-6 relative pb-20 z-10">
            <div class="bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 rounded-xl p-4 flex gap-3 items-start transition-colors duration-300 shadow-sm dark:shadow-none">
                ...
                <div class="flex flex-col gap-1">
                    <h3 class="text-sm font-bold text-rose-800 dark:text-rose-200 transition-colors duration-300">{{ $t('settings.account.phrases.warning_title') }}</h3>
                    <p class="text-[11px] text-rose-700/80 dark:text-rose-200/70 leading-relaxed transition-colors duration-300">{{ $t('settings.account.phrases.warning_desc') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-2.5">
                <div 
                    ...
                    :key="index"
                    class="group relative flex items-center gap-2 px-3 py-2.5 bg-white dark:bg-[#131315] border border-slate-200 dark:border-white/5 rounded-lg hover:border-indigo-300 dark:hover:border-white/10 transition-all shadow-sm dark:shadow-none select-none"
                >
                    <span class="text-[10px] font-mono text-slate-400 dark:text-zinc-600 group-hover:text-indigo-400 dark:group-hover:text-zinc-500 w-4 text-right transition-colors">{{ index + 1 }}</span>
            ...
            <button 
                @click="copyPhrases"
                class="flex items-center justify-center gap-2 py-3 rounded-xl border border-dashed border-slate-300 dark:border-zinc-700 hover:border-indigo-400 dark:hover:border-zinc-500 hover:bg-white dark:hover:bg-white/5 transition-all group cursor-pointer"
            >
                <span class="text-xs font-bold transition-colors" :class="copied ? 'text-emerald-600 dark:text-emerald-500' : 'text-slate-500 dark:text-zinc-400 group-hover:text-slate-800 dark:group-hover:text-zinc-200'">
                    {{ copied ? $t('settings.account.phrases.copy_success') : $t('settings.account.phrases.copy_action') }}
                </span>
        ...
        <div class="p-5 border-t border-slate-200 dark:border-white/5 bg-white dark:bg-[#09090b] relative transition-colors duration-300">
            ...
            >
                {{ $t('settings.account.phrases.btn_done') }}
            </button>
        </div>
    </div>
</template>
...
// Bu ekranda "tikla-gor" kapagi YOK: 24 kelime kosulsuz basiliyor. Yani gizlenme
// aninda yapilabilecek tek sey ust sayfaya donmektir -- v-if kapanir, bilesen
// unmount olur ve ebeveyndeki `decodedMnemonic` @clear ile null'a cekilir.
useSecretScreenGuard({
    onHide: () => { page.currentPage = 'settings_edit_account' },
})
