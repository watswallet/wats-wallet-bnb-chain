<template>
    <div class="w-full h-full max-w-[420px] mx-auto flex flex-col bg-slate-50 dark:bg-[#09090b] text-slate-900 dark:text-white font-sans relative overflow-hidden selection:bg-indigo-500/30 transition-colors duration-300">
        ...
        <!-- z-20 GEREKLI, z-10 DEGIL. Asagidaki icerik blogu `-mt-10` ile bu seridin
             uzerine 40px biniyor; ikisi de z-10 iken DOM'da sonra gelen (icerik) uste
             boyaniyor ve GERI BUTONUNUN buyuk bolumu tiklanmaz oluyordu.

             Butonun kendi z-40'i bunu COZMEZ: bu serit `relative z-*` oldugu icin KENDI
             yiginlama baglamini aciyor ve icerideki z-index yalnizca burada gecerli
             (ayni tuzak NetworkScopePill.vue'nun bas yorumunda da yazili).

             Hata GORUNMEZDI: binen alan icerigin bos ust kismi (icerik justify-center),
             yani ekran dogru gorunup tiklamayi yutuyordu. -->
        <div class="flex items-center justify-center px-5 pt-5 pb-3 relative border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-transparent backdrop-blur-md z-20 transition-colors duration-300">
            ...
            <h1 class="text-lg font-bold tracking-tight text-slate-900 dark:text-white transition-colors duration-300">{{ $t('settings.showTonKey.title') }}</h1>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-6 flex flex-col justify-center gap-8 relative -mt-10 z-10">
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <div class="text-center space-y-1">
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white transition-colors duration-300">{{ $t('settings.showTonKey.subtitle') }}</h2>

                    <i18n-t keypath="settings.showTonKey.desc" tag="p" class="text-xs text-slate-500 dark:text-zinc-500 max-w-55 mx-auto leading-relaxed transition-colors duration-300">
                        <template #account>
                            <span class="text-slate-800 dark:text-zinc-300 font-bold">{{ account.name }}</span>
                        </template>
                    </i18n-t>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-2">
                <div class="relative group">
                    <input
                        ...
                        :placeholder="$t('settings.showTonKey.placeholder_pass')"
                ...
                <div class="h-5 ml-1">
                    <p v-if="error" class="text-xs text-rose-600 dark:text-rose-500 flex items-center gap-1 animate-fade-in transition-colors duration-300">
                        ...
                        {{ $t('settings.showTonKey.error_wrong_pass') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="p-5 border-t border-slate-200 dark:border-white/5 bg-white dark:bg-[#09090b] relative transition-colors duration-300">
            ...
            >
                ...
                <span v-else>{{ $t('settings.showTonKey.btn_continue') }}</span>
            </button>
        </div>
    </div>
</template>
...
import { exportTonKey } from '../../../utils/ton/tonExport'
import Back from '../../Back.vue'

// ShowPrivateKey.vue'nin BIREBIR aynisi: ayni parola sorma akisi, ayni hata
// yonetimi. Tek fark: exportAccountPrivateKey yerine exportTonKey, ve emit edilen
// deger ham private key degil { secretKeyHex, friendly } nesnesi.
//
// Akis EVM ile AYNI uc adima cikariliyor (parola -> uyari -> anahtar): burasi
// yalnizca parolayi dogrulayip anahtari App.vue'ye emit ediyor, App.vue de EVM'de
// oldugu gibi 'settings_ton_key_disclaimer' sayfasina yonlendiriyor. Anahtar
// GORUNTULENMIYOR -- gosterim TonKey.vue'de, uyari adimindan SONRA.
const props = defineProps(['account'])
const emit = defineEmits(['ton-key'])
...
const unlock = async() => {
    ...
    try {
        ...
        if (!vaults || !walletSalt) {
            throw new Error("Cüzdan verisi bulunamadı")
        }
        ...
        // exportTonKey de tonIdentityForAccount uzerinden AYNI kasa/hesap eslesmesini
        // kullanir; yanlis hesabin anahtarinin gosterilmesi burada da mumkun degildir.
        const exported = await exportTonKey(masterKey, vaults, props.account)

        emit('ton-key', exported)
        page.currentPage = 'settings_ton_key_disclaimer'
