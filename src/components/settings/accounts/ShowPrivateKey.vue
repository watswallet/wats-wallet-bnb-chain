<template>
    <div class="w-full h-full max-w-[420px] mx-auto flex flex-col bg-slate-50 dark:bg-[#09090b] text-slate-900 dark:text-white font-sans relative overflow-hidden selection:bg-indigo-500/30 transition-colors duration-300">
        ...
        <div class="flex items-center justify-center px-5 pt-5 pb-3 relative border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-transparent backdrop-blur-md z-20 transition-colors duration-300">
            ...
            <h1 class="text-lg font-bold tracking-tight text-slate-900 dark:text-white transition-colors duration-300">{{ $t('settings.account.showPrivateKey.title') }}</h1>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-6 flex flex-col justify-center gap-8 relative -mt-10 z-10">
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <div class="text-center space-y-1">
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white transition-colors duration-300">{{ $t('settings.account.showPrivateKey.subtitle') }}</h2>
                    
                    <i18n-t keypath="settings.account.showPrivateKey.desc" tag="p" class="text-xs text-slate-500 dark:text-zinc-500 max-w-55 mx-auto leading-relaxed transition-colors duration-300">
                        <template #account>
            ...
            <div class="flex flex-col gap-2">
                <div class="relative group">
                    <input 
                        ...
                        :placeholder="$t('settings.account.showPrivateKey.placeholder_pass')" 
                ...
                <div class="h-5 ml-1">
                    <p v-if="error" class="text-xs text-rose-600 dark:text-rose-500 flex items-center gap-1 animate-fade-in transition-colors duration-300">
                        ...
                        {{ $t('settings.account.showPrivateKey.error_wrong_pass') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="p-5 border-t border-slate-200 dark:border-white/5 bg-white dark:bg-[#09090b] relative transition-colors duration-300">
            ...
            >
                ...
                <span v-else>{{ $t('settings.account.showPrivateKey.btn_continue') }}</span>
            </button>
        </div>
    </div>
</template>
...
import { exportAccountPrivateKey } from '../../../utils/deriveAccount'
import Back from '../../Back.vue'

const props = defineProps(['account'])
const emit = defineEmits(['private'])
...
const unlock = async() => {
    ...
    try {
        ...
        // Hesabin kendi derivationPath'i kullanilir ve turetilen adres hesabin
        // adresiyle dogrulanir; aksi halde kullaniciya baska bir hesabin anahtari
        // verilebilir. Ice aktarilmis hesaplar da bu yoldan disa aktarilir.
        const private_key = await exportAccountPrivateKey(masterKey, vaults, props.account)

        emit('private', private_key)
        page.currentPage = 'settings_private_key_disclaimer'
