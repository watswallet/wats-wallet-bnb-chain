...
import { createVault, createVaultWithPrivateKey, deriveMasterKey, randBytes } from '../../utils/crypto-utils'
...
const props = defineProps(['private_key', 'mnemonic', 'ton_mnemonic'])
...
const create = async () => {
    ...
    try {
        ...
        // EMNİYET: bu ekran yalnızca İLK cüzdan için (import akışları kasa yokken
        // buraya devrediyor). Mevcut kasalar varken walletSalt yazımı onların
        // anahtarını kalıcı olarak yok eder.
        ...
        if(props.private_key) {
            const wallet = new Wallet(props.private_key)
            await chrome.runtime.sendMessage({ type: 'UNLOCK_IMPORTED', privateKey: props.private_key, address: wallet.address })
            ...
            const vault = await createVaultWithPrivateKey(masterKey, props.private_key, account)
            user.vault = vault
            ...
            const vault = await createVault(masterKey, props.mnemonic, account)
            user.vault = vault

            vaults.push(vault)

        } else if(props.ton_mnemonic) {
            // TEK HESAP, IKI ZINCIR. Kural ImportPhrases.vue ile AYNI yardimcida:
            // kopyalanan bir kural birinde unutulur ve kullanici hangi kapidan
            // girdigine gore farkli bir cuzdan alirdi.
            const { tonVault, evmVault, account: tonAccount } = await buildHybridTonAccount(
                masterKey, props.ton_mnemonic, { name: 'Wats 1', existingVaults: vaults }
            )
        ...
        emit('confirmed', 'create_username')
