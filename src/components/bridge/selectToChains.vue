<template>
    <div class="fixed inset-0 flex items-center justify-center z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm px-4" @click="popups.bridge_to_network = false"> 
        ...
        >
            ...
            <div class="flex items-center justify-between px-5 pt-5 pb-3 relative z-10">
                <h4 class="font-bold text-slate-900 dark:text-white text-lg tracking-tight transition-colors duration-300">{{ $t('bridge.toChain.title') }}</h4>
                <button 
                    ...
                    @click="popups.bridge_to_network = false"
                >
            ...
            <div class="px-5 pb-2 relative z-10">
                <div class="relative group flex items-center w-full">
                    ...
                    <input 
                        ...
                        :placeholder="$t('bridge.toChain.search_placeholder')"
                    >
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar px-3 pb-3 space-y-1 relative z-10">
                <div v-if="filteredChains.length === 0" class="flex flex-col items-center justify-center py-12 text-slate-400 dark:text-zinc-500 transition-colors duration-300">
                    ...
                    <p class="text-sm font-medium">{{ $t('bridge.toChain.not_found') }}</p>
                </div>
                ...
                <button 
                    ...
                    :class="crypto.bridge.toChain?.chainId === chain.chainId
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col items-start">
                            <p class="text-sm font-semibold transition-colors duration-300" :class="crypto.bridge.toChain?.chainId === chain.chainId ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-zinc-300 group-hover:text-slate-900 dark:group-hover:text-white'">
                                {{ chain.name }}
                            </p>
                            ...
                            <div class="flex items-center gap-2 mt-0.5">
                                ...
                                <span v-if="chain.testnet" class="px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-500 text-[9px] font-bold uppercase transition-colors duration-300">
                                    {{ $t('bridge.toChain.testnet_label') }}
                                </span>
                            </div>
                        </div>
                    </div>

                    <div v-if="crypto.bridge.toChain?.chainId === chain.chainId" class="w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center shadow-[0_0_10px_rgba(16,185,129,0.2)] dark:shadow-[0_0_10px_rgba(16,185,129,0.4)] transition-all">
...
const filteredChains = computed(() => {
    // 1. Mevcut ağı hariç tut (Bridge from X -> to Y, where X != Y)
    //
    // Kopru CIFT eksenlidir: HEDEF zincir de akisi desteklemek zorunda. Filtre
    // olmadan hem TON hem Solana bu listeye giriyordu (Solana'nin `testnet` alani
    // bile yok, yani listelenen zincirlere kendiliginden dusuyor). TON hedef
    // secilirse LI.FI o zinciri hic tanimadigi icin ekran sessizce doner; Solana hedef
    // secilirse arka planda outToken base58 / toAddress EVM karisimi bir teklif
    // istegi cikar (bkz. background.js getBridgeQuote'daki AYNI kontrol).
    //
    // Kapi TEK yerde: chainsForFlow('bridge') -> chainSupportsFlow -> isEvm, ve
    // isEvm hem TON'u hem Solana'yi disliyor (chainKind.js/NON_EVM_KINDS).
    //
    ...
    let list = chainsForFlow('bridge').filter(chain => chain.chainId !== network.currentNetwork.chainId)
...
const selectChain = chain => {
    crypto.bridge.toChain = chain
    // Seçim yapıldıktan sonra bu modal kapanır, token seçim modalı (bridge_to) açılır
    popups.bridge_to_network = false
    popups.bridge_to = true
}
